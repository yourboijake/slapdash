...
from app.models.sql_models import ChatMessageForm, ChatMessage, get_message_history
...
async def home(request: Request, session: SessionDep) -> HTMLResponse:
  ...
  page = templates.TemplateResponse(
    "chat_home.jinja",
    {"request": request, "chat_metadata": chat_metadata, "chat_history": chat_history},
  )
  return page
  ...
  try:
    while True:
      ...
      if new_chat != None:
        ...
        session.commit()
        ChatHistoryRowOOBSwap = template_env.get_template(
          "chat.jinja"
        ).module.ChatHistoryRowOOBSwap  # type: ignore
